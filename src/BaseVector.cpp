...
BaseVector::BaseVector(std::vector<double> vector) {
    coordinates = vector;
}
...
BaseVector& BaseVector::operator*=(const BaseMatrix& rhs) {
    assert(this->dimension() <= rhs.rowSize());
    auto pm = BaseMatrix::make_matrix<1>(coordinates);
    auto rm = pm * rhs;
    auto rv = rm.toVector();
}
